...
struct FGameplayMessageListenerData
{
	...
	bool bListenForTargetActor = false;
};
...
public:
	 ...
	 * Broadcast a message on the specified channel
	 *
	 * @param TargetActor		The message target actor send to
	 * @param Channel			The message channel to broadcast on
	 * @param Message			The message to send (must be the same type of UScriptStruct expected by the listeners for this channel, otherwise an error will be logged)
	 */
	template <typename FMessageStructType>
	void BroadcastMessageToActor(AActor* TargetActor, FGameplayTag Channel, const FMessageStructType& Message)
	{
		const UScriptStruct* StructType = TBaseStructure<FMessageStructType>::Get();
		BroadcastMessageInternal(Channel, StructType, &Message, true, TargetActor);
	}
	...
	FGameplayMessageListenerHandle RegisterListenerToActor(AActor* TargetActor, FGameplayTag Channel, TFunction<void(FGameplayTag, const FMessageStructType&)>&& Callback, EGameplayMessageMatch MatchType = EGameplayMessageMatch::ExactMatch)
	{
		...
		const UScriptStruct* StructType = TBaseStructure<FMessageStructType>::Get();
		return RegisterListenerInternal(Channel, ThunkCallback, StructType, MatchType, true, TargetActor);
	}
	 ...
	 */
	template <typename FMessageStructType>
	FGameplayMessageListenerHandle RegisterListenerToActor(FGameplayTag Channel, AActor* TargetActor, FGameplayMessageListenerParams<FMessageStructType>& Params)
	{
		FGameplayMessageListenerHandle Handle;
		...
		if (Params.OnMessageReceivedCallback)
		{
			...
			const UScriptStruct* StructType = TBaseStructure<FMessageStructType>::Get();
			Handle = RegisterListenerInternal(TargetActor, Channel, ThunkCallback, StructType, Params.MatchType, true, TargetActor);
		}
...
protected:
	 ...
	 * Broadcast a message on the specified channel
	 *
	 * @param TargetActor		The message send to
	 * @param Channel			The message channel to broadcast on
	 * @param Message			The message to send (must be the same type of UScriptStruct expected by the listeners for this channel, otherwise an error will be logged)
	 */
	UFUNCTION(BlueprintCallable, CustomThunk, Category=Messaging, meta=(CustomStructureParam="Message", AllowAbstract="false", DisplayName="Broadcast Message To Actor"))
	void K2_BroadcastMessageToActor(AActor* TargetActor, FGameplayTag Channel, const int32& Message);

	DECLARE_FUNCTION(execK2_BroadcastMessageToActor);

private:
	// Internal helper for broadcasting a message
	void BroadcastMessageInternal(FGameplayTag Channel, const UScriptStruct* StructType, const void* MessageBytes, bool bSendToTargetActor = false, const AActor* TargetActor = nullptr);
