...
void UGameplayMessageSubsystem::BroadcastMessageInternal(FGameplayTag Channel, const UScriptStruct* StructType, const void* MessageBytes, bool bSendToTargetActor, const AActor* TargetActor)
{
	...
	// Check TargetActor Valid
	if(bSendToTargetActor && !IsValid(TargetActor))
	{
		UE_LOG(LogGameplayMessageSubsystem, Error, TEXT("BraoadcastMessage to invalid target actor! GameplayTag: %s"), *Channel.ToString())
	}
	...
	for (FGameplayTag Tag = Channel; Tag.IsValid(); Tag = Tag.RequestDirectParent())
	{
		if (const FChannelListenerList* pList = ListenerMap.Find(Tag))
		{
			...
			{
				// Only listen for global messages or personal messages.
				bool bListenGlobalMessage = !Listener.bListenForTargetActor && !bSendToTargetActor;
				bool bListenPersonalMessage = Listener.bListenForTargetActor && bSendToTargetActor && TargetActor == Listener.TargetActor;

				if(bListenGlobalMessage || bListenPersonalMessage)
				{
...
void UGameplayMessageSubsystem::K2_BroadcastMessageToActor(AActor* TargetActor, FGameplayTag Channel,
	const int32& Message)
{
	// This will never be called, the exec version below will be hit instead
	checkNoEntry();
...
DEFINE_FUNCTION(UGameplayMessageSubsystem::execK2_BroadcastMessageToActor)
{
	...
	if (ensure((StructProp != nullptr) && (StructProp->Struct != nullptr) && (MessagePtr != nullptr)) && IsValid(TargetActor))
	{
		P_THIS->BroadcastMessageInternal(Channel, StructProp->Struct, MessagePtr, true, TargetActor);
	}
}

FGameplayMessageListenerHandle UGameplayMessageSubsystem::RegisterListenerInternal(FGameplayTag Channel, TFunction<void(FGameplayTag, const UScriptStruct*, const void*)>&& Callback, const UScriptStruct* StructType, EGameplayMessageMatch MatchType, bool bListenForTargetActor, TWeakObjectPtr<AActor> TargetActor)
{
	...
	Entry.bListenForTargetActor = bListenForTargetActor;
